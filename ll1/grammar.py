...
class Grammar:
    ...
    def first(self, x):
        ...
        if isinstance(x, tuple):
            first_set = self.first_multiple(x)
        elif self.is_terminal(x):
            first_set = {x}  # Rule 1 and 2
        else:
            for production in self.productions_for(x):
                first_set = first_set.union(self.first(production))

        return sorted(first_set)

    # token es una tupla de simbolos
    def first_multiple(self, tokens):
        ...
        for t in tokens:
            first_token = self.first(t)
            first_set = first_set.union(first_token)
            if self.epsilon not in first_token:
                break
